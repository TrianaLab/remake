...
func (a *App) Login(ctx context.Context, user, pass string) error {
	return a.store.Login(ctx, user, pass)
}
...
func (a *App) Run(ctx context.Context, reference, target string) error {
	path, err := a.store.Pull(ctx, reference)
	if err != nil {
		return err
	}
	return a.runner.Run(ctx, path, target)
}
